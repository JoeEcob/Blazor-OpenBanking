namespace Spendy.Data.Loaders
{
    ...
    {
        public TransactionLoader(AuthService authService, TrueLayerAPI trueLayerApi, LiteDBDatastore dataStore) : base(authService, trueLayerApi, dataStore)
        {
        }

        protected override DateTime GetLastUpdateTime(Provider provider, string accountId = null)
        {
            return _dataStore.FindOne<Account>(x => x.AccountId == accountId).LastTransactionUpdate;
        }
        ...
        protected override async Task<Transaction[]> MapToClasses(Provider provider, TLTransaction[] data, string accountId = null)
        {
